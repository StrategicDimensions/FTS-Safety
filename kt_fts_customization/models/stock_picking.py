...
class StockPicking(models.Model):
    ...
    def do_new_transfer(self):
        ...
        if self.picking_type_id.name == 'Delivery Orders' and self.location_dest_id.id != vendor_loc_id.id:
            if self.origin:
                so_name = self.origin.split(':')[0] #self.origin[:5]
            sale = self.env['sale.order'].search([('name', '=', so_name)], limit=1)
            if not sale:
                raise ValidationError(('There is no sale order named - %s')%(so_name))
            if not sale.invoice_ids:
                raise ValidationError(('Invoice has been not created for this sale order %s')%(so_name))
    
            if self.partner_id.company_type == 'company':
                if self.partner_id.account_type == 'cod':
                     for invoice in sale.invoice_ids:
                         if invoice.state != 'paid':
                             raise ValidationError(('Invoice has been not Paid for this sale order %s')%(so_name))
                elif self.partner_id.account_type == 'account':
                    for invoice in sale.invoice_ids:
                        if invoice.state not in ['open','paid']:
                            raise ValidationError(('Invoice has been neither Validated nor Paid for this sale order %s')%(so_name))
            elif self.partner_id.company_type == 'individual':
                if self.partner_id.parent_id:
                    if self.partner_id.parent_id.account_type == 'cod':
                        for invoice in sale.invoice_ids:
                            if invoice.state != 'paid':
                                raise ValidationError(('Invoice has been not Paid for this sale order %s')%(so_name))
                    elif self.partner_id.parent_id.account_type == 'account':
                        for invoice in sale.invoice_ids:
                            if invoice.state not in ['open','paid']:
                                raise ValidationError(('Invoice has been neither Validated nor Paid for this sale order %s')%(so_name))
                else:
